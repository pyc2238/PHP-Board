...
<body>
  ...
  <div class="container">
    ...
    <input type="hidden" value="<?= $searchChoice ?>" name="tt">

    <div class="col-sm ">
    <div class="pagination float-left">  
          <?php if($startPage > 5 ): ?>
                <a href="<?= bdUrl("search.php",0,1, $searchChoice, $search) ?>">처음</a>
            <?php endif ?>    
    
            <?php if($startPage > 5 ): ?>
                <a href="<?= bdUrl("search.php",0,$page - 1, $searchChoice, $search) ?>"><</a>
            <?php endif ?>    
    
            <?php for($i = $startPage; $i <= $endPage; $i++) : ?>

                <?php if ($i == $page) : ?>
                    <a id="choice" href="<?= bdUrl("search.php",0,$i, $searchChoice, $search)?>"><b><?= $i ?></b></a>
                <?php else: ?>
                    <a href="<?= bdUrl("search.php",0,$i, $searchChoice, $search)?>"> <?= $i ?> </a>
                <?php endif ?>
                    
            <?php endfor ?>

            <?php if($endPage < $totalPages ) :?>
                <a href="<?= bdUrl("search.php",0, $i, $searchChoice, $search)?>">></a>
            <?php endif ?>    
            <?php if($endPage < $totalPages ) :?>
                <a href="<?= bdUrl("search.php",0,$totalPages, $searchChoice, $search)?>">끝</a>
            <?php endif ?> 
         </div><!--end of col-sm-->
    ...
    <?php if($uid) :?>
        ...
        <?php endif ?>
        <button class="btn btn-outline-warning  float-right" type="button" onclick="location.href='<?= bdUrl('community.php',0,0) ?>'"><i class="fa fa-list" style="font-size:15px;">목록</i></button>
